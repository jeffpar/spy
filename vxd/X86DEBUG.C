...
VOID x86Trap(PESF pesf, INT flDebug, INT iNum)
{
    ...
    // Special software interrupt processing here (BUGBUG: not functional
    // for WIN386 yet...)
...
PDES x86SelDesc(DWORD sel)
{
    ...
    if (sel & SEL_LDT) {
        ...
        return NULL;
#else
        if ((WORD)(sel & SEL_MASK) > dtrLDT.wLimit)
            return NULL;

        // Get ptr to descriptor

        return pLDT + (sel >> SEL_SHIFT);
#endif
    }
...
VOID x86SetByte(DWORD sel, DWORD off, BYTE b)
{
    ...
    if (sel == sel_Text)
        ...
        // Edit the descriptor so we're guaranteed write access
        TypeSave = pdes->bType;
        pdes->bType = SEG_DATA;
        // Because SEG_DATA defaults to DPL 0, we must also set the
        // RPL of the selector to 0 to avoid a privilege violation
        sel &= ~SEL_RPL3;
    }

    TrapSetByte(sel, off, b);

    // If the descriptor was edited, restore it
    if (TypeSave != -1)
        pdes->bType = (BYTE)TypeSave;
#endif
}
...
#ifdef WIN386
    if (bOp == OP_INT && bOp2 == 0x20 && (selCode & SEL_PROT32))
        offCur += 4;            // INT 20h in 32-bit CS has 4 extra bytes
#endif
...
#ifdef WIN386
        case 'l':
            x86DescDump(pLDT, &dtrLDT, "LDT", ++pchCmd, (WORD)off);
            break;

        case 's':
            if (pchAddr != pchArgs)
                x86StackDump(pesf, sel, off, cLines);
            else
                x86StackDump(pesf, pesf->SS, pesf->ESP, cLines);
            break;
#endif
        default:
...
INT x86Command(register PESF pesf, PCHAR pchCmd, PINT pflCommand)
{
    ...
    *pflCommand = 0;
    ...
    case 'l':
        if (*pch == '?') {
            printf("Label commands\n"
                   "\tlm\tlist maps\n"
                   "\tln\tlist nearest label\n"
                   "\tlo\tload symbol file\n"
            );
            break;
        }
        ...
        case 'n':
#ifdef WIN386
            SymDumpAddress(pesf, pch);
#endif
            break;
        case 'o':
#ifdef WIN386
            SymOpen(pch, NULL, TRUE);
#endif
            break;
        default:
            goto CommandError;
...
#ifdef WIN386
        if (_tolower(*pch) == 'm') {
            if (!VMMDumpCommand(pesf, pch+1))
                goto CommandError;
            break;
        }
        if (_tolower(*pch) == 'x' && _tolower(*(pch+1)) == 'd') {
            if (!VxDDumpCommand(pesf, pch+2))
                goto CommandError;
            break;
        }
#endif
    ...
    case '?':
        if (!*pch) {
            printf("Debugger commands\n"
                   ...
                   "\tvxd?\tvirtual device driver commands\n"
#endif
